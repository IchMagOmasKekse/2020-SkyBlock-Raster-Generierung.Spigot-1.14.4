...
public class IslandManager {
	...
	public static boolean loadProfile(Player p) {
		if(profiles.containsKey(p)) return false;
			...
			}else if(SkyFileManager.isMemberOfAnIsland(PlayerProfiler.getUUID(p))) {
				IslandProfile profile = new IslandProfile(SkyFileManager.getIslandWhereHeIsMemberOf(uuid), uuid);
				profiles.put(p, profile);	
			}else return false;
			return true;
		}
